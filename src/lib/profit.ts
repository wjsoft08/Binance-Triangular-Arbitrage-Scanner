...
export default async function createProfitFunc(
  ...
  const tickers = <Record<string, Ticker>>await getTickersData();
  const marketsData = <Record<string, Market>>await getMarketsData();

  function baseBudget(coin: string, markets: Record<string, any>) {
  ...
  return function checkProfit(
    pairsToTest: string[][],
    markets: Record<string, any>
  ) {
    // console.log("checkProfit triggered");
    const ret = [];
    const winners = [];
    for (let j = 0; j < pairsToTest.length; j += 1) {
      ...
      for (let i = 0; i < 3; i += 1) {
        ...
        if (markets[p12] && marketsData[p12]) {
          ...
          const bidPrice = new Decimal(markets[p12].bidPrice).toDecimalPlaces(
          ...
          orders.push({
            ...
            amount: budget,
            price: bidPrice,
            link: `https://www.binance.com/en/trade/${p12.replace("/", "_")}`,
            total: budget
              .mul(bidPrice)
              .toDecimalPlaces(
          ...
          const askPrice = new Decimal(markets[p21].askPrice).toDecimalPlaces(
      ...
      if (!fail) {
        const fee = new Decimal(FEE).mul(3);
